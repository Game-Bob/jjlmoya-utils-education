...
export const content: ToolLocaleContent<CurriculumProgressTrackerUI> = {
  ...
  seo: [
    ...
    { type: 'paragraph', html: 'Укажите общее число кредитов для выпуска и распределите оставшиеся предметы по реалистичным семестрам.' },
    { type: 'paragraph', html: 'Полученные кредиты показывают реальный прогресс, а запланированные предметы нужны только для будущей симуляции.' },
    { type: 'paragraph', html: 'Прогноз среднего балла зависит от предполагаемых оценок для оставшихся предметов.' },
    { type: 'paragraph', html: 'Сравнивайте несколько вариантов, не принимая один прогноз за гарантию.' },
    { type: 'paragraph', html: 'Требуемая оценка показывает результат, необходимый для достижения выбранной цели.' },
    { type: 'paragraph', html: 'Более лёгкий семестр помогает сохранить стабильность, а большая нагрузка может ускорить набор кредитов.' },
    { type: 'paragraph', html: 'При сравнении планов учитывайте доступное время и реальную сложность каждого предмета.' },
    { type: 'paragraph', html: 'Экспортируйте план перед крупными изменениями, чтобы сравнить прежние варианты.' },
    { type: 'paragraph', html: 'Статусы завершён, выполняется и запланирован помогают сохранить понятную симуляцию.' },
    { type: 'paragraph', html: 'Расчёт является общей ориентировочной оценкой и не заменяет консультацию в университете.' },
    { type: 'paragraph', html: 'Проверьте веса оценок, правила компенсации и требования выпуска в официальных документах.' },
    { type: 'paragraph', html: 'Обновляйте локальные данные после публикации каждого официального результата.' },
    { type: 'paragraph', html: 'Реалистичный план помогает измерять прогресс и заранее обращаться за поддержкой.' },
  ],
  faq: faqItems,
  howTo: howToItems,
  bibliography,
